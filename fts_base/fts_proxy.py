...
from . import cache_fixed_kwargs as tools
...
class fts_proxy(TransientModel):
    ...
    def _register_hook(self, cr):
        ...
        for plugin_type in fts_base_meta._to_register:
            ...
            if plugin_module not in pool._init_modules:
                continue
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None,
        ...
        if not context.get('fts_no_syntax_check') and not offset and not count:
            try:
                cr.execute("select to_tsquery('pg_catalog.simple', %s)",
                           (searchstring,))
            except psycopg2.ProgrammingError:
                raise except_orm(
                    _('Error'), _('You filled in an invalid searchstring!'))

        # TODO: if this search is limited, it is probably about scrolling and
        # we have cached the results of the initial nonlimited search. So
        # return that. Should look something like
        # if self.search.lookup(self, cr, args, 0, None, order, context,
        #                       count))
        # and works only in openerp6.1

        logger.debug(
            'offset: %s limit: %s order=%s count=%s' % (offset, limit,
                                                        order, count)
        )
        ...
        for search_plugin in fts_base_meta._plugins:
            ...
            res += search_plugin.search(
                cr, uid, args, order=order, context=context, count=count,
    ...
    def recreate_search_index(self, cr, uid, fts_object):
        ...
        self.pool.get('fts.proxy').create_init_tsvector_cronjob(
            cr, uid, fts_object)
    ...
    def init_tsvector_cronjob(self, cr, uid, fts_classname, context=None):
        ...
        for search_plugin in fts_base_meta._plugins:
            if (search_plugin._model) == fts_classname:
                logger.info(
                    'running _init_tsvector_column for ' + fts_classname)
                search_plugin._init_tsvector_column(self.pool, cr)
                logger.info(
                    'finished _init_tsvector_column for ' + fts_classname)
