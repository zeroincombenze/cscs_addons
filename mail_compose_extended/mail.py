...
class mail_mail(osv.Model):
    ...
    def _get_partner_access_link(self, cr, uid, mail, partner=None,
        ...
        link to action_mail_redirect action that will redirect to doc
        or Inbox """
        if context is None:
            context = {}
        if partner:
            ...
            return """<span class='oe_mail_footer_access'><small>%(access_msg)s
                      <a style='color:inherit' href='%(portal_link)s'>
                      %(portal_msg)s</a></small></span>""" % {
                'access_msg': _('about') if mail.record_name else _('access'),
                'portal_link': '',
                'portal_msg': '%s %s' % (
                    context.get('model_name',
                                ''),
                    mail.record_name) if mail.record_name else _(
                        'your messages'),
            }
        else:
            return None
    ...
    def send_get_mail_body(self, cr, uid, mail, partner=None, context=None):
        ...
        if mail.notification or (mail.model and mail.res_id):
            link = self._get_partner_access_link(
                cr, uid, mail, partner, context=context)
        if link:
            body = tools.append_content_to_html(
                body, link, plaintext=False, container_tag='div')
        return body

    def send_get_mail_reply_to(self, cr, uid, mail, partner=None,
                               context=None):
        ...
        # if model and res_id: try to use ``message_get_reply_to`` that returns
        # the document alias
        if mail.model and mail.res_id and \
                hasattr(self.pool.get(mail.model), 'message_get_reply_to'):
            email_reply_to = self.pool.get(mail.model).message_get_reply_to(
                cr, uid, [mail.res_id], context=context)[0]
        # no alias reply_to -> reply_to will be the email_from, only the email
        # part
        ...
        if email_reply_to and mail.model and mail.res_id:
            user_name = self.pool.get("res.users").read(
                cr, uid, [uid], fields=["name"], context=context)[0]
            if user_name:
                email_reply_to = _("%s <%s>") % (
                    user_name.get('name'), email_reply_to)
    ...
    def send_get_email_dict(self, cr, uid, mail, partner=None, context=None):
        ...
        reply_to = self.send_get_mail_reply_to(
            cr, uid, mail, partner=partner, context=context)
        body_alternative = tools.html2plaintext(body)

        # generate email_to, heuristic:
        # 1. if 'partner' is specified and there is a related document:
        #    Followers of 'Doc' <email>
        # 2. if 'partner' is specified, but no related document:
        #    Partner Name <email>
        # 3; fallback on mail.email_to that
        #    we split to have an email addresses list

        new_email_to = []
        partner_obj = self.pool.get('res.partner')
        partner_ids = partner_obj.search(
            cr, uid, [('name', '=', mail.record_name)], context=context)

        if partner_ids:
            for partner in partner_ids:
                foll = partner_obj.browse(
                    cr, uid, partner, context=context).message_follower_ids
                new_email_to.append(mail.email_to)
                if foll:
                    new_email_to.append(mail.email_to)
                    for partner_to_send in partner_obj.browse(
                            cr, uid, partner,
                            context=context).message_follower_ids:
                        if partner_to_send and mail.record_name:
                            # sanitized_record_name = re.sub(
                            #     r'[^\w+.]+', '-', mail.record_name)
                            email_to = _('%s <%s>') % (
                                partner_to_send.name, partner_to_send.email)
    ...
    def send(self, cr, uid, ids, auto_commit=False, recipient_ids=None,
        ...
        """ Sends the selected emails immediately, ignoring their current
            ...
            :param bool auto_commit: whether force a commit of the mail status
                after sending each mail (meant only for scheduler processing);
                should never be True during ordinary transactions
                (default: False)
            :param list recipient_ids: specific list of res.partner recipients.
                ...
                tune the sent email through ``send_get_mail_body`` and
                `send_get_mail_subject``.
        ...
        for mail in self.browse(cr, SUPERUSER_ID, ids, context=context):
            if mail.model:
                model_id = self.pool['ir.model'].search(
                    cr, SUPERUSER_ID, [('model', '=', mail.model)],
                    context=context)[0]
            ...
            try:
                ...
                for attach in mail.attachment_ids:
                    attachments.append(
                        (attach.datas_fname, base64.b64decode(attach.datas)))
                ...
                if mail.email_to:
                    email_list.append(self.send_get_email_dict(
                        cr, uid, mail, mail.res_id, context=context))
                else:
                    email_list.append(self.send_get_email_dict(
                ...
                for email in email_list:
                    ...
                    res = ir_mail_server.send_email(
                        cr, uid, msg, mail_server_id=mail.mail_server_id.id,
                        context=context)
                if res:
...
class mail_compose_message(osv.Model):
    ...
    def generate_email_for_composer(self, cr, uid, template_id, res_id,
                                    context=None):
        """ Call email_template.generate_email(), get fields relevant for
            mail.compose.message,
            transform email_cc and email_to into partner_ids """
        values = super(mail_compose_message, self).generate_email_for_composer(
            cr, uid, template_id, res_id, context=context)
        if context.get('active_model'):
            Values = [x.id for x in self.pool.get(
                context.get('active_model')).browse(
                cr, uid, res_id).message_follower_ids]
...
class mail_notification(osv.Model):
    ...
    def _notify(self, cr, uid, msg_id, partners_to_notify=None, context=None):
        """ Send by email the notification depending on the user preferences
          321  :param list partners_to_notify:
               optional list of partner ids restricting
        ...
        return super(mail_notification, self)._notify(cr, uid, msg_id,
                                                      partners_to_notify,
                                                      context)
