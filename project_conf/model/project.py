...
class project_task(orm.Model):
    ...
    def _message_get_auto_subscribe_fields(self, cr, uid, updated_fields,
        ...
        res = super(project_task, self)._message_get_auto_subscribe_fields(
    ...
    def send_mail_task_new_test(self, cr, uid, ids, context=None):
        '''
        Send mail automatically to change task to Backlog and Testing Leader.
        '''
        context = context or {}
        # Dont send context to dont get language of user in read method
        if ids.get('stage_id'):
            type = self.pool.get('project.task.type').read(
                cr, uid, ids['stage_id'][0], ['name'])
    ...
    def send_mail_task(self, cr, uid, ids, template, context=None):
        ...
        if template_ids:
            ...
            mail_ids.update(
                {'model': 'project.task', 'body': body_html,
    ...
    _columns = {
        'project_leader_id':
            fields.many2one('res.users', 'Project Leader',
                            help="""Person responsible of task review,
 when is in Testing Leader state. The person should review: Work Summary,
 Branch and Make Functional Tests.
 When everything works this person should change task to done."""),
    }
    _defaults = {
